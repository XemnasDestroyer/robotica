...
void SpecificWorker::computeDistances(const RoboCompLidar3D::TPoints &points) {
	...
	// Calcular distancia mínima en 360° (para espiral)
	for(const auto &point : points) {
		...
		// SECTOR FRONTAL: -FRONT_ANGLE_RANGE a +FRONT_ANGLE_RANGE
		if(std::abs(angle) <= params.FRONT_ANGLE_RANGE) {
			if(point.r < front && point.r > 10) {
				front = point.r;
			}
		}

		// SECTOR IZQUIERDO: 90° - SIDE_SECTION/2 a 90° + SIDE_SECTION/2
		float left_min_angle = M_PI/2 - params.SIDE_SECTION/2;
		float left_max_angle = M_PI/2 + params.SIDE_SECTION/2;
		if(angle >= left_min_angle && angle <= left_max_angle) {
			if(point.r < left && point.r > 10) {
				left = point.r;
			}
		}

		// SECTOR DERECHO: -90° - SIDE_SECTION/2 a -90° + SIDE_SECTION/2
		float right_min_angle = -M_PI/2 - params.SIDE_SECTION/2;
		float right_max_angle = -M_PI/2 + params.SIDE_SECTION/2;
		if(angle >= right_min_angle && angle <= right_max_angle) {
			if(point.r < right && point.r > 10) {
				right = point.r;
			}
		}
	}
...
std::tuple<State,float,float> SpecificWorker::Forward() {
    // Si hay MUCHO espacio libre, activar SPIRAL
...
std::tuple<State,float,float> SpecificWorker::Spiral()
{
	...
	{
		params.THETA = 0.f;
		return {State::FORWARD, 600.0f, 0.f};
	}

	// Parámetros de la espiral
	const float a = 30.0f;   // Radio inicial pequeño
	const float b = 15.0f;   // Separación más amplia
	const float v = 500.0f;  // Velocidad más alta

	params.THETA += 0.12f;   // Crecimiento angular más rápido
	float r = a + b * params.THETA;

	// ω = v / r (fórmula física correcta)
	float omega = v / r;
	omega = std::clamp(omega, 0.08f, 1.2f);
