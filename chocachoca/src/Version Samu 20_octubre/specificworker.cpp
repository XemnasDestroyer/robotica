...
{
	// Solo parar si choque inminente
	if(dist.front < 200.0f || dist.left < 150.0f || dist.right < 150.0f)
	{
		params.THETA = 0.f;
		return {State::TURN, 0.f, (dist.left < dist.right ? +1.0f : -1.0f)};
	}

	// ESPIRAL ULTRA-RÁPIDA
	params.THETA += 0.15f;

	// Velocidades casi máximas
	float advance = std::min(params.MAX_ADV_SPEED * 0.9f, 800.0f); // Casi velocidad máxima
	float rotation = std::max(0.4f, 0.8f - (params.THETA * 0.01f)); // Rotación decreciente

	// Límite de espiral muy amplio
	if(params.THETA > 50.0f) // Muchas vueltas
	{
		params.THETA = 0.f;
		return {State::FORWARD, params.MAX_ADV_SPEED * 0.8f, 0.f};
	}

	return {State::SPIRAL, advance, rotation};
}
