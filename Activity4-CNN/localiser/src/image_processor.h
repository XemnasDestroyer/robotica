...
check_number_in_image(RoboCompMNIST::MNISTPrxPtr mnist_proxy,
                      RoboCompCamera360RGB::Camera360RGBPrxPtr camera_proxy,
                      QLabel *label_img = nullptr)
        {
            if (!mnist_proxy || !camera_proxy)
            {
                qWarning() << "[ImageProcessor] Proxies not initialized";
                return {false, -1, 0};
            }

            try
            {
                // Get image only for visualization / width
                auto img = camera_proxy->getROI(-1, -1, -1, -1, -1, -1);
                int img_width = img.width;

                // Ask Python MNIST
                RoboCompMNIST::MNISTResult res = mnist_proxy->getNumber();

                if (res.number < 0)
                {
                    // No number → keep turning
                    return {false, -1, 1};
                }

                // Compute visual error
                int error_x = res.center - img_width / 2;

                const int tolerance = img_width / 15;
                int left_right = 0;

                if (error_x < -tolerance)
                    left_right = -1;
                else if (error_x > tolerance)
                    left_right = 1;

                // If centered → success
                if (left_right == 0)
                {
                    return {true, res.number, 0};
                }

                // Not centered yet → keep turning
                return {false, -1, left_right};
            }
            catch (const Ice::Exception &e)
            {
                qWarning() << "[ImageProcessor] MNIST error";
                return {false, -1, 0};
            }
        }*/
